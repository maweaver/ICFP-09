...
/**
 * Info about a step in the Eccentric Meet and Greet Problem
 */
case class EccentricMeetAndGreetInfo(stepNum: Int, score: Double, fuel: Double, sxEarth: Double, syEarth: Double, sxTarget: Double, syTarget: Double, radius: Double)

/**
 * Problem 1, Hohmann, described in section 6.1
 */
object EccentricMeetAndGreet
extends Problem {
  
  /**
   * Info on the steps that have been executed
   */
  private var info: List[EccentricMeetAndGreetInfo] = Nil

  /**
   * @inheritDoc
   */
  override def name = "Eccentric Meet and Greet"
  
  /**
   * @inheritDoc
   */
  override def configurations = 3001 :: 3002 :: 3003 :: 3004 ::  Nil
  
  /**
   * @inheritDoc
   */
  override def binary = "bin3"
  ...
  override def observe(stepNum: Int) {
    info ::= EccentricMeetAndGreetInfo(stepNum, 
